float getIterativeCSF(int year, Jets &jets, Jets &btags, TFile* sfFile, string variationName, TString sampleCategory){
    ...
    if(sampCat.find("fakes")!=std::string::npos){weight *= 1.00982;}
    if(sampCat.find("flips")!=std::string::npos){weight *= 0.97315;}
    if(sampCat.find("rares")!=std::string::npos){weight *= 0.95190;}
    if(sampCat.find("signal_tch")!=std::string::npos){weight *= 0.95390;}
    if(sampCat.find("signal_tuh")!=std::string::npos){weight *= 0.98371;}
