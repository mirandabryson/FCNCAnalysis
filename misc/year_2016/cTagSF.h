float getIterativeCSF(int year, Jets &jets, Jets &btags, TFile* sfFile, string variationName, TString sampleCategory){
    ...
    if(sampCat.find("fakes")!=std::string::npos){weight *= 0.98504;}
    if(sampCat.find("flips")!=std::string::npos){weight *= 1.04178;}
    if(sampCat.find("rares")!=std::string::npos){weight *= 0.96808;}
    if(sampCat.find("signal_tch")!=std::string::npos){weight *= 0.97603;}
    if(sampCat.find("signal_tuh")!=std::string::npos){weight *= 0.99163;}
