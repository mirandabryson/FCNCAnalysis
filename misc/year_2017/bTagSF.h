float getBtagEffFromFile(float pt, int mcFlavour){
    //TFile* f_btag_eff = new TFile("../../NanoTools/cpp/2017_eff.root");
    TFile* f_btag_eff = new TFile("./outputs/2017_bEff.root");
...
float getBSF(int year, Jets &jets, Jets &bjets){
    ...
    float btag_data = 1.;
    float btag_mc = 1.;
    //string csv_path = "../../NanoTools/NanoCORE/Tools/btagsf/csv/DeepJet_94XSF_V4_B_F.csv";
    string csv_path = "../../NanoTools/NanoCORE/Tools/btagsf/csv/DeepFlavour_94XSF_WP_V3_B_F.csv";
    // CSV object
    ...
    for ( auto bjet : bjets ){
        //skip objects in bjet collection with pt>40
        //these are already in the jet collection, so we don't want to overcount
